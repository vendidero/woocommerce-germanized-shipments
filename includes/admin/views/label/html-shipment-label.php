...
<div class="wc-gzd-shipment-label column column-spaced col-12 show-if show-if-provider show-if-provider-<?php echo esc_attr( $shipment->get_shipping_provider() ); ?>" data-shipment="<?php echo $shipment->get_id(); ?>">
    <h4><?php printf( _x( '%s Label', 'shipments', 'woocommerce-germanized-shipments' ), wc_gzd_get_shipping_provider_title( $shipment->get_shipping_provider() ) ); ?> <?php echo ( ( $shipment->has_label() && $shipment->get_tracking_id() ) ? Admin::get_shipment_tracking_html( $shipment ) : '' ); ?></h4>
	        ...
	        <?php if ( $label ) : ?>
                <div class="shipment-label-actions-wrapper shipment-label-actions-download">
	                ...
	                <?php if ( 'return' === $label->get_type() ) : ?>
                        <a class="send-shipment-label email" href="#"><?php _ex(  'Send to customer', 'shipments', 'woocommerce-germanized-shipments' ); ?></a>
	                <?php endif; ?>
                    ...
                    <a class="remove-shipment-label delete" data-shipment="<?php echo esc_attr( $shipment->get_id() ); ?>" href="#"><?php _ex(  'Delete', 'shipments', 'woocommerce-germanized-shipments' ); ?></a>
                </div>
	        <?php else: ?>
