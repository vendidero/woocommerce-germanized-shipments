...
<select class="shipment-packaging-select" id="shipment-packaging-<?php echo esc_attr( $shipment->get_id() ); ?>" name="shipment_packaging_id[<?php echo esc_attr( $shipment->get_id() ); ?>]">
	<option value=""><?php echo esc_html_x( 'None', 'shipments-packaging', 'woocommerce-germanized-shipments' ); ?></option>
	<?php
	foreach ( $shipment->get_selectable_packaging() as $packaging ) :
		...
		><?php echo esc_html( $packaging->get_title() ); ?><?php echo ( ! in_array( (int) $packaging->get_id(), $available_packaging_ids, true ) ? ' (' . esc_html_x( 'Does not fit', 'shipments', 'woocommerce-germanized-shipments' ) . ')' : '' ); ?></option>
	<?php endforeach; ?>

	<?php if ( ! $default_exists_in_list && $default_packaging ) : ?>
		<option value="<?php echo esc_attr( $default_packaging->get_id() ); ?>" <?php selected( $default_packaging->get_id(), $shipment->get_packaging_id(), true ); ?>><?php echo esc_html( $default_packaging->get_title() ); ?> (<?php echo esc_html_x( 'Does not fit', 'shipments', 'woocommerce-germanized-shipments' ); ?>)</option>
	<?php endif; ?>
</select>
