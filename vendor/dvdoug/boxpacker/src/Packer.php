    ...
    public function doVolumePacking(bool $singlePassMode = false, bool $enforceSingleBox = false): PackedBoxList
    {
        ...
        while ($this->items->count()) {
            ...
            try {
                ...
                /*
                var_dump("=== Best box: " . $bestBox->getBox()->getReference() . " (" . sizeof( $bestBox->getItems() ) . ") ===");
	            $count = 0;

	            foreach( $packedBoxesIteration as $iteration ) {
	            	$count++;
	                var_dump("===  " . $count . ": " . $iteration->getBox()->getReference() . " (" . sizeof( $iteration->getItems() ) . ") ===");
                }
                */
