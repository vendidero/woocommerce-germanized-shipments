...
abstract class REST {
	...
	protected function get_response( $url, $type = 'GET', $body_args = array(), $headers = array() ) {
		...
		return new Response( 500, array(), array(), new ShipmentError( 'rest-error', sprintf( _x( 'Error while trying to perform REST request to %s', 'shipments', 'woocommerce-germanized-shipments' ), $url ) ) );
	}
	...
	protected function parse_error( $response ) {
		...
		if ( isset( $body['message'] ) ) {
			...
			$error->add( $response->get_code(), _x( 'There was an unknown error calling the API.', 'shipments', 'woocommerce-germanized-shipments' ) );
		}
